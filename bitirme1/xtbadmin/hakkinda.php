...
if(isset($_POST['hizmet']))
{
	...
	if($dosya!="")
	{
		$resim_bul=mysql_fetch_array(mysql_query("select * from hizmetler where id=1"));
		$resim_sil=unlink("../images/hizmetlerimiz/".$resim_bul['resim']);
	
		$fotograf_duzenle_sorgu=mysql_query("update hizmetler set   hizmet='$hizmet', resim='$yeni_isim' where id=1");
		
		$gitti=move_uploaded_file($kaynak,$hedef);		
	}
	else
	{	
	$referans_duzenle_sorgu=mysql_query("update hakkinda set aciklama='$hizmet' where id='1'");
	}
...
$resim_bul=mysql_fetch_array(mysql_query("select * from hakkinda where id='1'"));
